...
from gait_in_eight.environments.honey_badger_cpg.viewer import MujocoViewer
from gait_in_eight.environments.honey_badger_cpg.control_functions.handler import get_control_function
from gait_in_eight.environments.honey_badger_cpg.command_functions.handler import get_command_function
from gait_in_eight.environments.honey_badger_cpg.sampling_functions.handler import get_sampling_function
from gait_in_eight.environments.honey_badger_cpg.initial_state_functions.handler import get_initial_state_function
from gait_in_eight.environments.honey_badger_cpg.reward_functions.handler import get_reward_function
from gait_in_eight.environments.honey_badger_cpg.termination_functions.handler import get_termination_function
from gait_in_eight.environments.honey_badger_cpg.domain_randomization.mujoco_model_functions.handler import get_domain_randomization_mujoco_model_function
from gait_in_eight.environments.honey_badger_cpg.domain_randomization.control_functions.handler import get_domain_randomization_control_function
from gait_in_eight.environments.honey_badger_cpg.domain_randomization.perturbation_functions.handler import get_domain_randomization_perturbation_function
from gait_in_eight.environments.honey_badger_cpg.observation_noise_functions.handler import get_observation_noise_function
from gait_in_eight.environments.honey_badger_cpg.terrain_functions.handler import get_terrain_function
from gait_in_eight.environments.honey_badger_cpg.trajectory_smoothing_functions.handler import get_trajectory_smoothing_function
from gait_in_eight.environments.honey_badger_cpg.central_pattern_generator.handler import get_central_patter_generator_function
...
class HoneyBadger(gym.Env):
    def __init__(self, seed, render,
                 ...
                 central_pattern_generator_type,
                 ...
                 add_goal_arrow, timestep, episode_length_in_seconds, total_nr_envs, kp, kd, action_space_mode,
                 cpu_id=None):
        ...
        self.training_reward_function = get_reward_function(reward_type, self)
        self.eval_reward_function = get_reward_function("track_xy", self)
        ...
        self.training_reward_function.init()
        self.eval_reward_function.init()
    ...
    def reset(self, seed=None):
        ...
        if self.eval:
            self.reward_function = self.eval_reward_function
        else:
            self.reward_function = self.training_reward_function
    ...
    def step(self, action):
        ...
        if self.action_space_mode == "cpg_default":
            if self.command_type == "max_velocity":
                stride_length = (self.action_space.high - self.action_space.low)[0]
                x_vel_avg = np.mean(self.command_function.performance_history)
                x_vel_avg = np.abs(x_vel_avg)
                frequency = x_vel_avg / stride_length + 1.5
                frequency = np.clip(frequency, 2.0, 4.0)
                print(frequency, x_vel_avg, stride_length)
                offset = self.central_pattern_generator_function.step(frequency)
            else:
        ...
        for i in range(self.nr_substeps):
            ...
            if self.action_space_mode == "cpg_residual":
                ...
                torques_action_ik = self.kp * (action_ik - self.data.qpos[7:]) + self.kd * (action - self.data.qvel[6:])
            if self.central_pattern_generator_type == "none":
                torques = self.control_function.process_action(action)
            else:
                torques = self.control_function.process_action(action, offset)
            if self.action_space_mode == "cpg_residual":
                torques = (torques + torques_action_ik) / 2
                action = action_copy
    ...
    def get_observation(self):
        ...
        # cpg = self.central_pattern_generator_function.step()
        observation[obs_idx.CPG] = self.cpg
