...
class TrackXReward:
    ...
    def reward_and_info(self, info, done):
        ...
        # current_global_linear_velocity = self.env.next_observation[obs_idx.TRUNK_LINEAR_VELOCITIES]
        ...
        target_velocity_reward = 0
        x_current_velocity = current_local_linear_velocity[0]
        x_target_velocity = desired_local_linear_velocity_xy[0]
        if x_current_velocity >= x_target_velocity and x_current_velocity <= 2.0 * x_target_velocity:
            target_velocity_reward = 1
        elif x_current_velocity <= -1 * x_target_velocity or x_current_velocity >= 4.0 * x_target_velocity:
            target_velocity_reward = 0
        else:
            target_velocity_reward = 1 - np.abs(x_current_velocity - x_target_velocity) / (2 * x_target_velocity)
        yaw_current_velocity = np.abs(current_local_angular_velocity[2])
        # Total reward

        pitch_roll_position_norm = np.sum(np.square(orientation_euler[:2]))
        pitch_roll_position_penalty = pitch_roll_position_norm * -10
        torque_norm = np.sum(np.square(self.env.data.ctrl))
        torque_penalty = -torque_norm * 0.0003
        yaw_penalty = -0.1 * np.abs(current_local_angular_velocity[2])

        reward = (
            target_velocity_reward + yaw_penalty + pitch_roll_position_penalty + torque_penalty
        )
        reward *= 10
        ...
        info[f"reward/velocity_tracking"] = target_velocity_reward
        info[f"reward/yaw_penalty"] = yaw_penalty
        info[f"reward/pitch_roll_position_penalty"] = pitch_roll_position_penalty
        info[f"reward/torque_penalty"] = torque_penalty
