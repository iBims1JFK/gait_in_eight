...
from gait_in_eight.algorithms.crossq.default.policy import get_policy
from gait_in_eight.algorithms.crossq.default.critic import get_critic
from gait_in_eight.algorithms.crossq.default.entropy_coefficient import (
    EntropyCoefficient,
)
from gait_in_eight.algorithms.crossq.default.replay_buffer import ReplayBuffer
from gait_in_eight.algorithms.crossq.default.hindsight_experience_replay_buffer import HindsightExperienceReplayBuffer
from gait_in_eight.algorithms.crossq.default.rl_train_state import RLTrainState

from gait_in_eight.environments.honey_badger_jtp.environment import HoneyBadger
from gait_in_eight.environments.honey_badger_cpg.environment import HoneyBadger as HoneyBadgerCPG
...
class CrossQ:
    def __init__(self, config, env, run_path, writer):
        ...
        self.state_history_length = config.algorithm.state_history_length
        self.hindsight_replay = config.algorithm.hindsight_replay
        self.action_space_mode = config.algorithm.action_space_mode

        self.skip = np.arange(self.learning_starts - 1, self.learning_starts + 2)
        ...
        (
            self.key,
            policy_key,
            policy_batch_renorm_key,
            critic_key,
            critic_batch_renorm_key,
        ...
        state = jnp.empty((1, self.env.single_observation_space.shape[0] * self.state_history_length))
        ...
        if self.save_model:
            ...
            best_model_check_point_handler = orbax.checkpoint.PyTreeCheckpointHandler(
                aggregate_filename=self.best_model_file_name
            )
            self.best_model_checkpointer = orbax.checkpoint.Checkpointer(
                best_model_check_point_handler
            )

    def train(self):
        @jax.jit
        def get_action(
            policy_state: TrainState, state: np.ndarray, key: jax.random.PRNGKey
        ):
        ...
        def update_critic(
            ...
            def loss_fn(
                ...
                y = reward + self.gamma * (1 - terminated) * (
                    min_next_q_target - alpha * next_log_prob
                )
        ...
        def update_policy_and_entropy_coefficient(
            ...
            def loss_fn(
                ...
                entropy_coefficient_params: flax.core.FrozenDict,
                state: np.ndarray,
                subkey: jax.random.PRNGKey,
            ):
                # Policy loss
            ...
            (loss, (policy_state_update, metrics)), (
                policy_gradients,
                entropy_gradients,
            ) = grad_loss_fn(
        ...
        steps_metrics = {}
        past_states = np.zeros((self.state_history_length, self.env.single_observation_space.shape[0]))
        past_states[-1] = state
        ...
        if self.action_space_mode == "default":
            ...
            action_space_shape = 13
        elif self.action_space_mode == "cpg_residual":
            action_space_shape = 24
        ...
        while global_step < self.total_timesteps:
            ...
            if global_step <= self.learning_starts:
                ...
                action = (processed_action - self.env_as_low) / (
                    self.env_as_high - self.env_as_low
                ) * 2.0 - 1.0
            else:
                action, self.key = get_action(self.policy_state, past_states.flatten(), self.key)
                processed_action = self.get_processed_action(action)

            # if (self.skip == global_step).any():
            #     processed_action = np.array([np.zeros(12)])
            end_calc_acting = time.time()
            next_state, reward, terminated, truncated, info = self.env.step(
                jax.device_get(processed_action.reshape(1, action_space_shape))
            )
            end_env_step = time.time()
            done = terminated | truncated
            actual_next_state = next_state.copy()
            past_states[:-1] = past_states[1:]
            past_states[-1] = next_state.copy()
            for i, single_done in enumerate(done):
                if single_done:
                    actual_next_state[i] = self.env.get_final_observation_at_index(
                        info, i
                    )
                    saving_return_buffer.append(
                        self.env.get_final_info_value_at_index(
            ...
            if not self.hindsight_replay:
                ...
                replay_buffer.add(state, actual_next_state, action, reward, terminated, truncated)
            ...
            time_metrics_collection.setdefault("time/acting_time", []).append(
                acting_end_time - start_time
            )

            # What to do in this step after acting
            should_learning_start = global_step >= self.learning_starts
            if should_learning_start and terminated:
                self.nr_terminated += 1
            should_optimize_critic = should_learning_start
            should_optimize_policy = (
                should_learning_start
            )
            should_evaluate = (
                global_step % self.evaluation_frequency == 0
                and self.evaluation_frequency != -1
            )
            should_try_to_save = (
                should_learning_start and self.save_model and dones_this_rollout > 0
            )
            ...
            if should_optimize_policy:
                (
                    ...
                    self.key,
                ) = update_policy_and_entropy_coefficient(
                    self.policy_state,
                    self.critic_state,
                    self.entropy_coefficient_state,
                    batch_states,
            ...
            time_metrics_collection.setdefault("time/optimizing_time", []).append(
                optimizing_end_time - acting_end_time
            )
            ...
            if should_evaluate:
                ...
                eval_nr_episodes = 0
                while True:
                    processed_action = get_deterministic_action(
                    ...
                    for i, single_done in enumerate(done):
                        if single_done:
                            ...
                            evaluation_metrics_collection.setdefault(
                                ...
                                self.env.get_final_info_value_at_index(
                                    info, "episode_length", i
                                )
                            )

                            for key, info_value in self.env.get_logging_info_dict(info).items():
                                splitted_key = key.split("/")
                                if splitted_key[0] == "eval":
                                    evaluation_metrics_collection.setdefault(key, []).append(
                                        self.env.get_final_info_value_at_index(
                                            info, key, i
                                        )
                                    )
            ...
            time_metrics_collection.setdefault("time/evaluating_time", []).append(
                evaluating_end_time - optimizing_end_time
            )
            ...
            time_metrics_collection.setdefault("time/saving_time", []).append(
                saving_end_time - evaluating_end_time
            )

            time_metrics_collection.setdefault("time/sps", []).append(
                self.nr_envs / (saving_end_time - start_time)
            )

            # Logging
            if should_log:
                Thread(target=self.logging, args=(time_metrics_collection, optimization_metrics_collection, evaluation_metrics_collection, step_info_collection, steps_metrics, global_step, nr_critic_updates, nr_policy_updates, nr_episodes)).start()
                time_metrics_collection = {}
                step_info_collection = {}
                optimization_metrics_collection = {}
                evaluation_metrics_collection = {}
            
    def logging(self, time_metrics_collection, optimization_metrics_collection, evaluation_metrics_collection, step_info_collection, steps_metrics, global_step, nr_critic_updates, nr_policy_updates, nr_episodes):
        simulation_steps = step_info_collection.get("simulation_step")[-1]
        step_info_collection.pop("simulation_step")


        self.start_logging(simulation_steps)
        
        steps_metrics["steps/nr_simulation_steps"] = simulation_steps
        ...
        if step_info_collection:
            ...
            for info_name in info_names:
                metric_group = (
                    "rollout"
                    if info_name in ["episode_return", "episode_length"]
                    else "env_info"
                )
                metric_dict = (
                    rollout_info_metrics
                    if metric_group == "rollout"
                    else env_info_metrics
                )
                mean_value = np.mean(step_info_collection[info_name])
                if mean_value == mean_value:  # Check if mean_value is NaN
                    metric_dict[f"{metric_group}/{info_name}"] = mean_value
        metric_dict["env_info/env_info/nr_terminated"] = self.nr_terminated
        time_metrics = {
            key: np.mean(value)
            for key, value in time_metrics_collection.items()
        }
        optimization_metrics = {
            key: np.mean(value)
        ...
        evaluation_metrics = {
            key: np.mean(value)
            for key, value in evaluation_metrics_collection.items()
        }
        combined_metrics = {
            **rollout_info_metrics,
            **evaluation_metrics,
    ...
    def save(self):
        ...
        os.rename(
            f"{self.save_path}/tmp/{self.best_model_file_name}",
            f"{self.save_path}/{self.best_model_file_name}",
        )
    ...
    def load(config, env, run_path, writer, explicitly_set_algorithm_params):
        ...
        check_point_handler = orbax.checkpoint.PyTreeCheckpointHandler(
            aggregate_filename=checkpoint_file_name
        )
        checkpointer = orbax.checkpoint.Checkpointer(check_point_handler)

        loaded_algorithm_config = checkpointer.restore(checkpoint_dir)[
            "config_algorithm"
        ]
    ...
    def test(self, episodes):
        ...
        for i in range(episodes):
            ...
            episode_return = 0
            state = np.zeros((self.state_history_length, self.env.single_observation_space.shape[0]))
            state[-1], _ = self.env.reset()
            while not done:
                processed_action = get_action(self.policy_state, state.flatten())
                next_state, reward, terminated, truncated, info = self.env.step(
                    jax.device_get(jnp.array([processed_action]))
                )
                state[:-1] = state[1:]
                state[-1] = next_state
                done = terminated | truncated
    ...
    def set_train_mode(self): 
        for env in self.env.envs:
            env = env.unwrapped
            if isinstance(env, HoneyBadger) or isinstance(env, HoneyBadgerCPG):
                env.set_eval_mode(False)
            else:
                print("Not HoneyBadger", env)
